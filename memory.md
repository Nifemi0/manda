...
- The UI and documentation distinguish the UTC-resetting daily budget from the owner-set cumulative onchain total cap. Payment principal is user-funded; paymasters sponsor eligible gas only. The live Robinhood ceiling has `4599999999999` wei remaining after the verified payment and the `1 wei` audit transaction.
...
- USDG testnet support has now been added in code. Official USDG contracts were read from Paxos documentation and checked through both testnet RPCs for bytecode, `USDG` symbol, and 6 decimals. Asset-bound policy signatures, ERC-20 spend hooks, service-side token balance and recipient checks, and asset-separated daily budgets are implemented. No USDG mandate/payment was submitted and the public Vercel/VPS deployment is not updated yet.
- The standard SDK USDG hook does not bind the transfer recipient onchain; only the authenticated Manda service checks it. This limitation must remain explicit until a recipient-bound validation contract/module is added.
